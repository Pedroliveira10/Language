...
export function ExpandedExerciseRenderer(exercise, { speechSupported, index, total, completed }) {
  const translation = exercise.translation
    ? `<p class="translation"><strong>Translation:</strong> ${escapeHtml(exercise.translation)}</p>`
    : '';
  const listen = exercise.audioText
    ? `<button class="button secondary" type="button" data-listen-exercise="${escapeHtml(exercise.id)}">🔊 Listen</button>`
    : '';
  let interaction = '';

  if (exercise.type === 'multipleChoice') {
    interaction = `<div class="answer-options">${exercise.options.map((option) =>
      `<button class="answer-option" type="button" data-answer-exercise="${escapeHtml(exercise.id)}" data-answer="${escapeHtml(option)}">${escapeHtml(option)}</button>`).join('')}</div>`;
  ...
  return `<section class="exercise-card ${completed ? 'is-complete' : ''}" data-exercise-card="${escapeHtml(exercise.id)}">
    <div class="exercise-kicker">Activity ${index + 1} of ${total} · ${escapeHtml(exercise.cefrLevel)} · ${escapeHtml(exercise.type)} · <span data-exercise-status>${completed ? 'Complete ✓' : 'Not complete'}</span></div>
